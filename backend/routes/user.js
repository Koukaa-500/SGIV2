...
router.post('/operation', auth, async (req, res) => {
  try {
    const userId = req.user._id; // Extract user ID from the token
    const { symbol, price, quantityOrdered, change, owner } = req.body;
    ...
    user.operation.push(operationEntry);
    await user.save();

    res.status(200).json({ message: 'operation entry added successfully' });
  } catch (error) {
    console.error('Error adding operation entry:', error);
    res.status(500).json({ message: 'Server error' });
  }
});
router.get('/operations', auth, async (req, res) => {
  try {
    const userId = req.user._id; // Extract user ID from the token
    const user = await User.findById(userId).select('operation');
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
    res.status(200).json(user.operation);
  } catch (error) {
