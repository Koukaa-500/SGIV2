...
export class ProductService {
  private baseUrl = 'http://192.168.1.199:3000/product/stocks';
  private baseUrl1 = 'http://192.168.1.199:3000/user';
  private apiUrl = 'http://192.168.1.199:3000/product';
  ...
  async toggleFavorite(symbol: string, isFavorite: boolean): Promise<Observable<any>> {
    ...
    try {
      ...
      return this.http.post<any>(`http://192.168.1.199:3000/user/favorite1`, requestPayload, { headers }).toPromise();
    } catch (error) {
  ...
  async getFavoriteStocks(): Promise<Observable<void>> {
    ...
    return this.http.get<string[]>(`http://192.168.1.199:3000/user/favorite`, { headers }).pipe(
